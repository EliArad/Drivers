...
namespace TCA6416Api
{
    public class TCA6416
    {
        ...
        private void Write(Command command, IO_BITS dataPort)
        {
            ...
            data[1] = Convert.ToByte((byte)dataPort);
        ...
        public void SetIODirection(PORTS port, IO_BITS bits)
        {
            bits = ~bits;
            switch (port)
            {
                case PORTS.PORT0:                    
                    Write(Command.Configuration0, bits);
                break;
                case PORTS.PORT1:
                    Write(Command.Configuration1, bits);
                break;
            }
        }
        ...
        public void WriteIO(IO_BITS b, bool val)
        {
            WriteIO(PORTS.PORT0, b, val);
            WriteIO(PORTS.PORT1, b, val);
        }
